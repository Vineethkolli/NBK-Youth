...
import { RefreshCcw, Clock, GitBranch } from 'lucide-react';
...
export default function GithubActionsMonitor() {
  ...
  // Filters affecting only the cards
  const [metricFilter, setMetricFilter] = useState('currentMonth'); 
  ...
  const fetchMetrics = useCallback(
    async (filter = metricFilter, workflow = workflowFilter) => {
      try {
        setLoading(true);
        const res = await axios.get(
          `${API_URL}/api/monitor/github/actions/metrics?filter=${encodeURIComponent(filter)}&workflow=${encodeURIComponent(workflow)}`
        );
  ...
  const fetchAll = useCallback(async () => {
    setLoading(true);
    try {
      await Promise.all([fetchWorkflows(), fetchMetrics(metricFilter, workflowFilter)]);
    } finally {
      setLoading(false);
    }
  }, [fetchWorkflows, fetchMetrics, metricFilter, workflowFilter]);

  useEffect(() => {
    fetchAll();
  }, []); 

  // When filters change, only re-fetch metrics
  useEffect(() => {
    fetchMetrics(metricFilter, workflowFilter);
  }, [metricFilter, workflowFilter, fetchMetrics]);
    ...
    <div className="bg-white p-6 rounded-xl shadow-lg max-w-6xl mx-auto space-y-6 font-sans">
      ...
      <div className="flex flex-wrap gap-3 items-center">
        <div className="flex items-center gap-2">
          ...
          <button
            onClick={() => setMetricFilter('currentMonth')}
            className={`px-3 py-1 rounded-lg font-medium ${
              metricFilter === 'currentMonth' ? 'bg-indigo-600 text-white' : 'bg-gray-100 text-gray-800'
            }`}
          >
            Month
          </button>
          <button
            onClick={() => setMetricFilter('overall')}
            className={`px-3 py-1 rounded-lg font-medium ${
              metricFilter === 'overall' ? 'bg-indigo-600 text-white' : 'bg-gray-100 text-gray-800'
            }`}
          >
            Overall
          </button>
  ...
  <table className="min-w-full divide-y divide-gray-200 text-sm">
    <thead className="bg-gray-100 sticky top-0">
      <tr>
        ...
        {/* MODIFICATION: Align header text to the left */}
