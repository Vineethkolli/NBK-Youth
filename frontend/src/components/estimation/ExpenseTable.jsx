        ...
        <thead className="bg-gray-50">
          <tr>
            ...
            {visibleColumns.previousAmount && (
              <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase">Previous Amount</th>
            )}
            {visibleColumns.presentAmount && (
              <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase">Present Amount</th>
            )}
            {visibleColumns.others && (
              <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase">Others</th>
            )}
        ...
        <tbody className="bg-white divide-y divide-gray-200">
          {expenses.map((expense, index) => (
            <tr key={expense._id}>
              ...
              {visibleColumns.previousAmount && (
                <td className="px-6 py-4 whitespace-nowrap text-sm notranslate">{expense.previousAmount}</td>
              )}
              {visibleColumns.presentAmount && (
                <td className="px-6 py-4 whitespace-nowrap text-sm notranslate">{expense.presentAmount}</td>
              )}
              {visibleColumns.others && (
                <td className="px-6 py-4 whitespace-nowrap text-sm">{expense.others}</td>
              )}
