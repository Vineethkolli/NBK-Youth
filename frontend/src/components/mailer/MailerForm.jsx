...
function MailerForm({
  ...
  /*
   * Load scheduled email into edit/reschedule mode.
   */
  useEffect(() => {
    ...
    setScheduleDate(
      toIstInput(editingSchedule.scheduledAt)
    );
  ...
  const openSchedulePicker = () => {
    if (!scheduleDate && editingSchedule?.scheduledAt) {
      setScheduleDate(
        toIstInput(editingSchedule.scheduledAt)
      );
    }
  ...
  const validatePayload = () => {
    if (
      ...
      toast.error(
        'Subject, body, and footer are required'
      );
      return false;
    }
  ...
  const sendNow = async () => {
    ...
    try {
      ...
      const { data } = await axios.post(
        `${API_URL}/api/mailer/send`,
        payload
      );

      toast.success(
        data?.message || 'Email send started'
      );

      if (onSent && data?.history) {
        onSent(data.history);
      }
  ...
  const scheduleEmail = async () => {
    ...
    try {
      ...
      toast.success(
        data?.message || 'Email scheduled'
      );

      if (onScheduled && data?.schedule) {
...
<button
  ...
  onClick={() => {
    if (scheduleMode) {
      cancelSchedulePicker();
    } else {
      openSchedulePicker();
    }
  }}
  disabled={isSubmitting}
  className={`flex-1 rounded-md border px-4 py-2 text-sm font-medium shadow-sm transition-colors ${
    scheduleMode
